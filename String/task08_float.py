...
def str_to_number_float(string):
    number = 0
    if 'point' in string:
        string = string.split(' point ')
        n = len(str(str_to_number(string[-1])))
        number += str_to_number(string[0]) + (str_to_number(string[-1])) / (10 ** (n))
    else:
        number += str_to_number(string)
    return(number)

# seven hundred eighty nine thousand one hundred twenty three point one hundred nineteen thousand six hundred forty five
...
print(string)
print(str_to_number_float(string))
