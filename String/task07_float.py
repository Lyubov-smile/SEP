...
def convert_number_string_float(number):
    number_string = ''
    if number.isnumeric():
        number = int(number)
        number_string = convert_number_string(number)
    elif '.' in number:
        number = number.split('.')
        number[0] = int(number[0])
        number[-1] = int(number[-1])
        number_string = convert_number_string(number[0]) + ' point ' + convert_number_string(number[-1])
    return number_string


number = input("input a number: ")
print(convert_number_string_float(number).capitalize())
